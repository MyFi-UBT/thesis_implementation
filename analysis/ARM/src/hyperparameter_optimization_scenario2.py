    ...
    try:
        ...
        for idx, c in enumerate(consequents):
            if c[0] == "Label = 1":
                ...
                # Beginning to check if the condition which contains the relevant process details is met.
                # If this condition is met, the relevant rule is found.
                pos_y_regex = r"(Obj\d) Positions Y.*?(\d+[.]\d+|\d+).*?(\d+[.]\d+|\d+)"

                objectsWithIntervallFound = []
                deviationPos = {}

                if len(result['antecedents'][idx]) > 1:
                    continue
                for res in result['antecedents'][idx]:
                    intervall_search = re.search(pos_y_regex, res)
                    if intervall_search:
                        if intervall_search.group(1) == "Obj3" or intervall_search.group(1) == "Obj4":
                            continue
                        if float(intervall_search.group(2)) >= 576 and float((intervall_search.group(3))) <= 647:

                            if not intervall_search.group(1) in deviationPos:
                                deviationPos[intervall_search.group(1)] = 0

                            if float((intervall_search.group(3))) <= 642:
                                # Deviation of the found interval borders compared to the ideal ones. The value 1
                                # refers to the fact that the ideal borders are part of the rule. The value 0 means
                                # that a maximum deviation from the ideal borders is found (= The interval borders
                                # have a maximal distance from the ideal ones, i.e. are in the middle of the perfect
                                # interval.)
                                deviationPos[intervall_search.group(1)] = max(1 - (((float(
                                    intervall_search.group(2)) - 576) + (642 - float(intervall_search.group(3)))) / (
                                                                                               642 - 576)),
                                                                              deviationPos[intervall_search.group(1)])
                            else:
                                deviationPos[intervall_search.group(1)] = max(1 - (((float(
                                    intervall_search.group(2)) - 576) + (647 - float(intervall_search.group(3)))) / (
                                                                                               647 - 576)),
                                                                              deviationPos[intervall_search.group(1)])
                            objectsWithIntervallFound.append(intervall_search.group(1))
                            print("I found a matching y-position for")
                            print(intervall_search.group(1))
                            print(intervall_search.group(2))
                            print(intervall_search.group(3))

                # if not empty, when there is a statement about an object.
                if len(objectsWithIntervallFound) != 0:
                    print("I found an appropriate rule :)")
                    outerDeviationPos = 0
                    for o in objectsWithIntervallFound:
                        # select best value regarding the interval borders, i.e., minimal deviations
                        outerDeviationPos = max(outerDeviationPos, deviationPos[o])
                    ruleIsPresent = True

#        print(ruleIsPresent, ": if True, relevant rule is present")
#        print(nrOfLabel1, "rules with label1")
        if ruleIsPresent and nrOfLabel1 != 0:
            print("my ret value is:")
            print((1 + outerDeviationPos) / nrOfLabel1)
            return (1 + outerDeviationPos) / nrOfLabel1
        else:
            return 0
    except KeyError:
        return 0
...
def start_my_study(scenario_nr, data_path, algorithm, prune_value):
    ...
    if prune_value:
        study_name = "scen2_" + str(scenario_nr) + "-" + algorithm + "-pruned-" + "nrt" + str(parameters['n_trials'])  # Unique identifier of the study.
    else:
        study_name = "scen2_" + str(scenario_nr) + "-" + algorithm + "-" + "nrt" + str(parameters['n_trials'])  # Unique identifier of the study.
...
data_path = os.path.join(os.path.abspath(os.path.dirname(
    os.path.dirname(os.path.dirname(current_dir))) + '/data_generator/structured_data/generated_data/scenario2'))
...
for i in range(1, 2):
    data_path = os.path.join(os.path.abspath(data_path + "/scenario_2_" + str(i)))
